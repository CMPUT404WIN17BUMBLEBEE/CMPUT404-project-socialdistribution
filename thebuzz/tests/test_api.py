...
class post_tests(TestCase):
    ...
    def test_comment(self):
        ...
        response = self.client.post('/api/posts/' + str(post_id) + '/comments/', content_type='application/json',
        ...
        self.assertIsInstance(dateutil.parser.parse(comment['published']), datetime,
                              "Published is not in datetime format")
        self.assertEqual(comment['author']['id'], str(Profile.objects.get(user__username='test').id),
                         "Associated author does not match")
